...
Frontend interface for the **Korra AI Agent** system.

This application provides a browser-based chat interface built with
Next.js for interacting with a LangGraph-powered backend agent.
It connects to the Korra backend service and enables real-time,
tool-augmented conversations through a streaming chat experience.

The UI communicates with a LangGraph server exposing a `messages`
state and is configured by default to connect to the Korra backend
running locally via Docker Compose.

---

> Based on the LangGraph Agent Chat UI project, adapted here as the
> frontend component of the Korra AI agent system.
